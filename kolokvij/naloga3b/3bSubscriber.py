...
#   1. podrocje == "matematika"                      -> binding key: "matematika.#"
#   2. podrocje == "biologija" IN podpodrocje == "deljenje" -> binding key: "biologija.deljenje.*"
#   3. vrsta == "ja_ne"                              -> binding key: "#.ja_ne"
#
# Wildcard pravila za topic exchange:
#   "*"  ujame tocno ENO besedo
#   "#"  ujame NULA ali VEC besed
...
def main():
    ...
    queue_result = channel.queue_declare(queue='', exclusive=True)  # ustvarimo ekskluzivni queue z nakljucnim imenom; exclusive=True: queue se izbrise ko se povezava zapre
    ...
    for binding_key in BINDING_KEYS:  # za vsak pogoj ustvarimo svojo vezavo
        channel.queue_bind(
            exchange=EXCHANGE,       # exchange na katerega se vezemo
            queue=my_queue,          # nas lokalni queue
            routing_key=binding_key  # vzorec po katerem RabbitMQ filtrira sporocila
        )
    ...
    def callback(ch, method, properties, body):  # funkcija se poklic ko prejmemo vprasanje
        ...
        vprasanje = body.decode('utf-8')   # dekodiramo besedilo vprasanja iz bytes v string
        ...
        ch.basic_ack(delivery_tag=method.delivery_tag)  # potrdimo RabbitMQ da smo sporocilo uspesno obdelali (brez tega bi ga RabbitMQ poslal znova)

    channel.basic_qos(prefetch_count=1)  # fair dispatch: prejmemo eno vprasanje naenkrat za hitrejsi odziv
    channel.basic_consume(queue=my_queue, on_message_callback=callback)  # registriramo callback funkcijo za nas queue
